...
def calculate_costs_for_external_components(mymodel):
    ...
    if mymodel['external_components'] is not None:
        ...
        # Check if all external component names are unique, otherwise throw an error. Therefore first get all
        ...
        for this_ext_comp_name in ext_comp_names:
            if ext_comp_names.count(this_ext_comp_name) != 1:
                raise ValueError(
                    'External component name "{}" is not unique, please name components unique.'.format(
                        this_ext_comp_name))
        ...
        for this_ext_comp in mymodel['external_components']:
            ...
            this_comp_module = importlib.import_module('smooth.components.external_component_' + this_ext_comp_name)
