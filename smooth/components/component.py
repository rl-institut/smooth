...
class Component:
    ...
    def update_var_emissions(self, results, sim_params):
        ...
        if self.variable_emissions is not None:
            ...
            self.results['variable_emissions'][sim_params.i_interval] = this_dependency_value * \
                self.variable_emissions
    ...
    def get_foreign_state_value(self, components, index=None):
        ...
        if not is_fs_found:
            raise ValueError(
                'Foreign state couldn\'t be found, please check the fs names of the supply component.')
