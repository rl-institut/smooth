...
class PemElectrolyzer (Component):
    ...
    def __init__(self, params):
        ...
        # The efficiency curve for the system efficiency over the load point of a PEM electrolyzer, according to:
        ...
        # System efficiency load break points in terms of hydrogen production (e.g. 0.05 --> 5 %) [-]
        ...
        # System efficiency break points in terms of hydrogen production (e.g. 0.05 --> 5 %) [-]
        self.bp_eff_h2_prod = [0, 0.2157, 0.4762, 0.563, 0.6303, 0.6751, 0.6947, 0.6863, 0.6779,
                0.6583, 0.6387, 0.6218, 0.6022, 0.5854, 0.5798]

        # The (approximated) efficiency curve for the waste heat efficiency over the load point of a PEM electrolyzer,
        # according to: Ni, M. et.al. 'Energy and exergy analysis of hydrogen production by a proton exchange membrane
        # (PEM) electrolyzer plant' 2008 and Waidhas, M. 'Electrolyzer technology - the Siemens view' 2016.
        ...
        self.bp_elec_consumed_h2_prod = [this_bp * self.power_max for this_bp in self.bp_load_h2_prod]
        self.bp_elec_consumed_waste_heat = [this_bp * self.power_max for this_bp in self.bp_load_waste_heat]
        ...
        for i_bp in range(len(self.bp_load_h2_prod)):
            this_hydrogen_production = \
                (self.bp_elec_consumed_h2_prod[i_bp] * self.bp_eff_h2_prod[i_bp])/(self.heating_value * 1000)
        ...
        for i_bp in range(len(self.bp_load_waste_heat)):
            ...
            this_waste_heat_energy = self.bp_elec_consumed_waste_heat[i_bp] * self.bp_eff_waste_heat[i_bp]
            self.bp_waste_heat_energy.append(this_waste_heat_energy)

        # While we will create two oemof components, one for the electrolyzer producing hydrogen and one for the
        # electrolyzer producing waste heat energy, and make a constraint that both inflows of electricity have to be
        # the same, each component will get only half the amount of electricity. Therefore we need to make a list of
        # electricity consumed that is halfed [Wh]
        self.bp_elec_consumed_h2_prod_half = [this_bp / 2 for this_bp in self.bp_elec_consumed_h2_prod]
        self.bp_elec_consumed_waste_heat_half = [this_bp / 2 for this_bp in self.bp_elec_consumed_waste_heat]

        # Save the two models to set constraints later (h2 for hydrogen production and th for waste heat energy).
        self.model_h2 = None
        self.model_th = None
    ...
    def create_oemof_model(self, busses, model):
        # Create the non-linear oemof component. The PEM electrolyzer has to be modelled as two components, while the
    ...
    def update_constraints(self, busses, model_to_solve):
        # Set a constraint so that the electricity inflow of the hydrogen production and the waste heat energy part are
        # always the same (which is necessary while the piecewise linear transformer cannot have two outputs yet and
